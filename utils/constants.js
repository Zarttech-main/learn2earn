export const assignments = [
    {
        id: 1,
        reward: 100,
        name: "C++ Quiz Test",
        added_by: "Abdul",
        questions: [
            {
                id: 1,
                question: "What is the full form of RTTI.",
                options: [
                    {
                        id: 1,
                        text: "Runtime type identification",
                    },
                    {
                        id: 2,
                        text: "Runtime template identification",
                    },
                    {
                        id: 3,
                        text: "Robust Template Type Inheritance",
                    },
                    {
                        id: 4,
                        text: "None of the above.",
                    },                    
                ],
                correct_option_id: 2,
            },
            {
                id: 2,
                question: "What is the outpout of the following program?",
                img: require("../public/tests/1.png"),
                options: [
                    {
                        id: 1,
                        text: "0 1",
                    },
                    {
                        id: 2,
                        text: "0 2",
                    },
                    {
                        id: 3,
                        text: "0 8",
                    },
                    {
                        id: 4,
                        text: "Compile Error",
                    },
                    
                ],
                correct_option_id: 1,
            },
            {
                id: 3,
                question: "The programs machine instructions are store in __ memory segment",
                options: [
                    {
                        id: 1,
                        text: "Data",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 2,
                        text: "Stack",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 3,
                        text: "Heap",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 4,
                        text: "Code",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    
                ],
                correct_option_id: 3,
            },
            {
                id: 4,
                question: "The default executable generation on UNIX for a C++ program is ___",
                options: [
                    {
                        id: 1,
                        text: "a.exe",
                    },
                    {
                        id: 2,
                        text: "a",
                    },
                    {
                        id: 3,
                        text: "a.out",
                    },
                    {
                        id: 4,
                        text: "out.a",
                    },
                    
                ],
                correct_option_id: 3,
            },
            {
                id: 5,
                question: "Which operator is required to be overloaded as member function only?",
                options: [
                    {
                        id: 1,
                        text: "_",
                    },
                    {
                        id: 2,
                        text: "_ _",
                    },
                    {
                        id: 3,
                        text: "++ (PostFix version)",
                    },
                    {
                        id: 4,
                        text: "=",
                    },
                    
                ],
                correct_option_id: 3,
            },
            {
                id: 5,
                question: "Following is the invalid inclusion of a file to the current program. Identify it",
                options: [
                    {
                        id: 1,
                        text: "#include <file>",
                    },
                    {
                        id: 2,
                        text: "#include file",
                    },
                    {
                        id: 3,
                        text: "#include < file ",
                    },
                    {
                        id: 4,
                        text: "All of the above are invalid",
                    },
                    
                ],
                correct_option_id: 3,
            },
        ]
    },
    {
        id: 2,
        reward: 100,
        name: "Fitness",
        added_by: "Nelson",
        questions: [
            {
                id: 1,
                question: "What is the meaning of fitness?",
                options: [
                    {
                        id: 1,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 2,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 3,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 4,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    
                ],
                correct_option_id: 2,
            },
            {
                id: 2,
                question: "What is the meaning of fitness?",
                options: [
                    {
                        id: 1,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 2,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 3,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 4,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    
                ],
                correct_option_id: 1,
            },
            {
                id: 3,
                question: "What is the meaning of fitness?",
                options: [
                    {
                        id: 1,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 2,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 3,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 4,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    
                ],
                correct_option_id: 3,
            },
            {
                id: 4,
                question: "What is the meaning of fitness?",
                options: [
                    {
                        id: 1,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 2,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 3,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 4,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                ],
                correct_option_id: 2,
            }
        ]
    },
    {
        id: 3,
        reward: 100,
        name: "Fitness",
        added_by: "Nelson",
        questions: [
            {
                id: 1,
                question: "What is the meaning of fitness?",
                options: [
                    {
                        id: 1,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 2,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 3,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 4,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    
                ],
                correct_option_id: 2,
            },
            {
                id: 2,
                question: "What is the meaning of fitness?",
                options: [
                    {
                        id: 1,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 2,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 3,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 4,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    
                ],
                correct_option_id: 1,
            },
            {
                id: 3,
                question: "What is the meaning of fitness?",
                options: [
                    {
                        id: 1,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 2,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 3,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 4,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    
                ],
                correct_option_id: 3,
            },
            {
                id: 4,
                question: "What is the meaning of fitness?",
                options: [
                    {
                        id: 1,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 2,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 3,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 4,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                ],
                correct_option_id: 2,
            }
        ]
    },
    {
        id: 4,
        reward: 100,
        name: "Fitness",
        added_by: "Nelson",
        questions: [
            {
                id: 1,
                question: "What is the meaning of fitness?",
                options: [
                    {
                        id: 1,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 2,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 3,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 4,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    
                ],
                correct_option_id: 2,
            },
            {
                id: 2,
                question: "What is the meaning of fitness?",
                options: [
                    {
                        id: 1,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 2,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 3,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 4,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    
                ],
                correct_option_id: 1,
            },
            {
                id: 3,
                question: "What is the meaning of fitness?",
                options: [
                    {
                        id: 1,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 2,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 3,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 4,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    
                ],
                correct_option_id: 3,
            },
            {
                id: 4,
                question: "What is the meaning of fitness?",
                options: [
                    {
                        id: 1,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 2,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 3,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 4,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                ],
                correct_option_id: 2,
            }
        ]
    },
    {
        id: 5,
        reward: 100,
        name: "Fitness",
        added_by: "Nelson",
        questions: [
            {
                id: 1,
                question: "What is the meaning of fitness?",
                options: [
                    {
                        id: 1,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 2,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 3,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 4,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    
                ],
                correct_option_id: 2,
            },
            {
                id: 2,
                question: "What is the meaning of fitness?",
                options: [
                    {
                        id: 1,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 2,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 3,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 4,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    
                ],
                correct_option_id: 1,
            },
            {
                id: 3,
                question: "What is the meaning of fitness?",
                options: [
                    {
                        id: 1,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 2,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 3,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 4,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    
                ],
                correct_option_id: 3,
            },
            {
                id: 4,
                question: "What is the meaning of fitness?",
                options: [
                    {
                        id: 1,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 2,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 3,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                    {
                        id: 4,
                        text: "Being physically fit",
                        img: "https://images.unsplash.com/photo-1601422407692-ec4eeec1d9b3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1625&q=80"
                    },
                ],
                correct_option_id: 2,
            }
        ]
    },
]

export const users = [
    {
        username: "tea",
        password: "pass123",
        isTeacher: true,
    },
    {
        username: "tea1",
        password: "pass123",
        isTeacher: true,
    },
    {
        username: "user",
        password: "pass123",
    },
    {
        username: "user2",
        password: "pass123",
    }
]

export const earnings = [
    {
        amount: 0.3,
        totalConfirmations: 0,
        timestamp: "27-12-2021 09:11:00",
    },
    {
        amount: 0.1,
        totalConfirmations: 2,
        timestamp: "24-12-2021 08:33:00",
    },
    {
        amount: 0.05,
        totalConfirmations: 2,
        timestamp: "23-12-2021 08:22:00",
    },
    {
        amount: 0.02,
        totalConfirmations: 2,
        timestamp: "20-12-2021 02:22:00",
    },
    {
        amount: 0.04,
        totalConfirmations: 3,
        timestamp: "18-12-2021 09:29:00",
    },
]

export const products = [
    {
       name:"Men's Wear [Jacket]",
       url:"http://images.thenorthface.com/is/image/TheNorthFace/236x204_CLR/mens-better-than-naked-jacket-AVMH_LC9_hero.png",
       price: 0.3
    },
    {
       name:"Women's Jacket",
       url:"http://images.thenorthface.com/is/image/TheNorthFace/236x204_CLR/womens-better-than-naked-jacket-AVKL_NN4_hero.png",
       price: 0.2
    },
    {
       name:"WOMEN'S SINGLE-TRACK SHOE",
       url:"http://images.thenorthface.com/is/image/TheNorthFace/236x204_CLR/womens-single-track-shoe-ALQF_JM3_hero.png",
      price: 0.6
    },
    {
       name:"Enduro Hydration Pack",
       url:"http://images.thenorthface.com/is/image/TheNorthFace/236x204_CLR/enduro-boa-hydration-pack-AJQZ_JK3_hero.png",
      price: 0.1
    },
    {
       name:"Men Tshirt",
       url:"http://images.thenorthface.com/is/image/TheNorthFace/236x204_CLR/mens-better-than-naked-jacket-AVMH_LC9_hero.png",
      price: 0.09
    },
    {
       name:"Women Nice Wear",
       url:"http://images.thenorthface.com/is/image/TheNorthFace/236x204_CLR/womens-better-than-naked-jacket-AVKL_NN4_hero.png",
      price: 0.8
    },
    {
       name:"Nice Lady's Shoe",
       url:"http://images.thenorthface.com/is/image/TheNorthFace/236x204_CLR/womens-single-track-shoe-ALQF_JM3_hero.png",
       price: 0.3
    },
    {
       name:"Hydration Pack",
       url:"http://images.thenorthface.com/is/image/TheNorthFace/236x204_CLR/enduro-boa-hydration-pack-AJQZ_JK3_hero.png",
       price: 0.89
    }
 ]